...
# Create filter_stocks.txt if it doesn't exist
if not os.path.exists('filter_stocks.txt'):
    with open('filter_stocks.txt', 'w') as f:
        f.write("# Add stock symbols to filter here\n")
...
def calculate_cosine_similarity_simple(returns_series, window=20):
    ...
    similarities = []
    for i in range(len(returns_series) - window + 1):
        current_window = returns_series.iloc[i:i+window].values
        if i == 0:
            similarities.extend([np.nan] * window)
            continue
            
        # Compare with previous window
        prev_window = returns_series.iloc[max(0, i-window):i].values
        if len(prev_window) == window:
            # Simple correlation instead of cosine similarity
            similarity = np.corrcoef(current_window, prev_window)[0,1]
            similarities.append(similarity if not np.isnan(similarity) else 0)
        else:
            similarities.append(np.nan)
    
    # Pad with NaN for the beginning
    padding = [np.nan] * (len(returns_series) - len(similarities))
    return pd.Series(padding + similarities, index=returns_series.index)
...
def generate_enhanced_features(stock_data):
    ...
    bbands = stock_data.ta.bbands()
    features['BB_Upper'] = bbands['BBU_5_2.0']
    features['BB_Middle'] = bbands['BBM_5_2.0']
    features['BB_Lower'] = bbands['BBL_5_2.0']
    features['BB_Width'] = (features['BB_Upper'] -
    ...
    features['ATR'] = stock_data.ta.atr()
...
def train_model(features, labels):
    ...
    feature_importance = analyze_feature_importance(model, features.columns)
...
def calculate_dynamic_threshold(similarity_scores, percentile=95):
    valid_scores = [s for s in similarity_scores if not np.isnan(s)]
    if not valid_scores:
        return 0.5
    return np.percentile(valid_scores, percentile)

def score_stock(stock_data, features, model_prediction, weights=None):
    score = 0
    signals = {}
    ...
    signals['BB'] = 1 if stock_data['close'].iloc[-1] < features['BB_Lower'] else (
        -1 if stock_data['close'].iloc[-1] > features['BB_Upper'] else 0)
    ...
    signals['ML_Model'] = model_prediction - 1  # Convert 0, 1, 2 to -1, 0, 1

    # Use simplified cosine similarity
    returns = np.log(stock_data['close'] / stock_data['close'].shift(1)).dropna()
    cosine_sim = calculate_cosine_similarity_simple(returns)
    if not cosine_sim.empty and not np.isnan(cosine_sim.iloc[-1]):
        dynamic_threshold = calculate_dynamic_threshold(cosine_sim.dropna())
        signals['Cosine_Similarity'] = 1 if cosine_sim.iloc[-1] > dynamic_threshold else 0
    else:
        signals['Cosine_Similarity'] = 0

    if weights is None:
        weights = {
            'RSI': 1, 'MACD': 1, 'BB': 1, 'MA_Cross': 1, 'Volume_Spike': 1,
            'Price_Momentum': 1.5, 'ML_Model': 1.5, 'Cosine_Similarity': 1.5
        }
...
def evaluate_stock(df, symbol, min_adv, scoring_threshold=2):
    ...
    try:
        ...
        stock_data = df[['TIMESTAMP'] + required_columns]
        stock_data = stock_data.rename(columns={
            f'{symbol}_OPEN': 'open',
            f'{symbol}_HIGH': 'high',
            f'{symbol}_LOW': 'low',
            f'{symbol}_CLOSE': 'close',
            f'{symbol}_TOTTRDQTY': 'volume'
        })
        ...
        latest_features = features.iloc[-1].to_frame().T
        prediction = model.predict(latest_features)[0]

        # Calculate final score
        final_score, final_signals = score_stock(
            stock_data, features.iloc[-1], prediction)
...
def filter_stocks(input_file, output_file_filtered, output_file_remaining):
    # Create a set of stocks to filter
    filter_stocks = set()
    try:
        with open('filter_stocks.txt', 'r') as f:
            for line in f:
                ...
                if line and not line.startswith('#'):
                    filter_stocks.add(line)
    except FileNotFoundError:
        logging.warning("filter_stocks.txt not found, skipping filtering")
        return
...
def main():
    ...
    top_stocks = get_top_stocks_by_adv(df, n=500)  # Reduced for testing
    ...
    output_file_filtered = 'filtered_stock_signals.csv'
    output_file_remaining = 'remaining_stock_signals.csv'
